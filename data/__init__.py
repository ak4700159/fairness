...
from fair_setup import set_env
...
def crawl(keyword):
    crawlers  = {
        "korea" : KoreaCrawler(keyword=keyword, site="korea"),
        ...
        "google" : GoogleCrawler(keyword=keyword, site="google")
    }

    total_results = []
    for crawler in crawlers.values():
        crawler.run()
        total_results.append(crawler.results)
...
def analyze(articles):
    ...
    for idx, article in enumerate(articles):
        try:
            response = chain.invoke({"user_prompt" : "민생 지원", "title" : article['title'], "content" : article['content'], "format_instructions" : output_parser.get_format_instructions()})
        except:
            print("LLM 응답 오류 발생")
        results.append({
            **article.to_dict(),
...
def save_to_local(results):
    # 현재 시간 표현, 폴더명에 들어가게됨
    KST = timezone(timedelta(hours=9))
    kst_now = dt.now(KST)
    datetime = kst_now.strftime("%m/%d")

    # 절대 경로로 결과 내용 저장
    current_dir = os.path.dirname(os.path.abspath(__file__))  # 현재 파일 위치: e.g. FAIRNESS/data/crawler/
    save_path = os.path.join(current_dir, "dumy")  # -> FAIRNESS/data/dumy/
    with open(save_path + f"{datetime}_{keyword}_result.json", 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    # 크롤링한 기사 분석 및 평가
    results = analyze(articles=articles)

    # 분석한 내용 DB server에 저장 + 로컬에도 저장
    save_to_local(results=results)
